...
CREATE INDEX `group_memberships_groupId_idx` ON `group_memberships` (`group_id`);--> statement-breakpoint
CREATE INDEX `group_memberships_userId_idx` ON `group_memberships` (`user_id`);--> statement-breakpoint
CREATE TABLE `groups` (
	...
	`extra` text,
	`created_at` text DEFAULT CURRENT_TIMESTAMP,
	FOREIGN KEY (`parent_id`) REFERENCES `groups`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
CREATE TABLE `todos` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
